...
const Product = async ({ params }: { params: { slug: string } }) => {
    ...
    return (
        <div className="grid max-xl:gap-7 sm:grid-cols-2 py-8">
            <div className="w-full h-full max-h-[604px] max-w-[604px]">
                <BlurImage src={product.images[0].url} alt={product.title} />
            </div>
            <div>
                <h1 className="mb-4 flex-auto text-4xl font-bold tracking-tight">
                    {product.title}
                </h1>
                <div className="mb-4 font-medium">
                    <span
                        className={`inline ${
                            product.salePrice && "text-neutral-500 line-through"
                        }`}
                    >
                        {settings.currency}
